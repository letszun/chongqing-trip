const STORE='cqTripAppV4';
let state={};
try{state=JSON.parse(localStorage.getItem(STORE)||localStorage.getItem('cqTripAppV2')||localStorage.getItem('cqTripAppV1')||'{}')}catch(e){state={}}
...
function nav(active){const items=[['index.html','◷','Plan','plan'],['spots.html','⌖','Spots','spots'],['eat.html','●','Eat','eat'],['money.html','¥','Money','money'],['talk.html','中','Talk','talk'],['check.html','✓','Check','check']];return `<nav class="bottom">${items.map(([href,icon,label,key])=>`<a class="nav ${active===key?'active':''}" href="${href}"><span class="nav-icon">${icon}</span>${label}</a>`).join('')}</nav>`}
